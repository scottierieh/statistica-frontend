...
def create_spatial_weights_matrix(n, structure='rook', normalize=True):
    N = n * n
    W = np.zeros((N, N))
    for i in range(n):
        for j in range(n):
            ...
            if structure == 'queen':
                if i > 0 and j > 0: W[idx, (i-1)*n + (j-1)] = 1
                if i > 0 and j < n-1: W[idx, (i-1)*n + (j+1)] = 1
                if i < n-1 and j > 0: W[idx, (i+1)*n + (j-1)] = 1
                if i < n-1 and j < n-1: W[idx, (i+1)*n + (j+1)] = 1
    if normalize:
...
def main():
    try:
        ...
        # Assuming grid data for simplicity if no W is provided
        n_side = int(np.sqrt(len(data)))
        if n_side * n_side != len(data):
            raise ValueError("Data size is not a perfect square for grid-based W matrix generation.")
            
        W = create_spatial_weights_matrix(n=n_side, structure='queen', normalize=True)
        ...
        results = {
            "coefficients": {
                "rho": model.rho,
                **{f"beta_{i}": v for i, v in enumerate(model.beta)}
            },
            "sigma2": model.sigma2,
