...
class RankingConjointAnalyzer:
    ...
    def _calculate_fit_metrics(self):
        ...
        self.pseudo_r2 = 1 - (self.log_likelihood_value / null_ll)
    ...
    def calculate_utility_ranges(self) -> Dict[str, float]:
        """Calculate utility range for each attribute"""
        if self.part_worths is None:
            self.calculate_part_worths()
        
        return self.utility_ranges

    def predict_market_share(self, products: List[Dict[str, str]]) -> Dict[str, float]:
        """
        Predict market share for a set of product configurations
        
        Args:
            products: List of product configurations, each as dict of {attribute: level}
        
        Returns:
            Dictionary mapping product index to predicted market share
        """
        if not self.fitted:
            raise RuntimeError("Model not fitted")
    ...
    def simulate_price_sensitivity(self, 
                                   price_attribute: str, 
                                   base_product: Dict[str, str],
                                   price_range: List[str]) -> Dict[str, float]:
        """
        Simulate market share changes across different price points
        
        Args:
            price_attribute: Name of the price attribute
            base_product: Base product configuration
            price_range: List of price levels to simulate
        
        Returns:
            Dictionary mapping price level to predicted market share
        """
        if price_attribute not in self.attributes:
            raise ValueError(f"Price attribute '{price_attribute}' not found in model")
        
        competitor_product = {attr: self.baselines[attr] for attr in self.attributes.keys()}
        ...
        for price in price_range:
            ...
            shares = self.predict_market_share([test_product, competitor_product])
            results[price] = shares["Product_1"]
        
        return results

    def calculate_rank_distribution(self) -> Dict[str, Dict[int, float]]:
        """Calculate the distribution of ranks for each profile"""
        if self.ranking_data is None:
            return {}
        
        rank_dist = {}
        for profile_id in self.ranking_data['profile_id'].unique():
            profile_ranks = self.ranking_data[
                self.ranking_data['profile_id'] == profile_id
            ]['rank'].values
            
            rank_counts = pd.Series(profile_ranks).value_counts(normalize=True) * 100
            rank_dist[str(profile_id)] = rank_counts.to_dict()
        
        return rank_dist

    def calculate_average_ranks(self) -> Dict[str, float]:
        """Calculate average rank score for each profile"""
        if self.ranking_data is None:
            return {}
        ...
        for profile_id in self.ranking_data['profile_id'].unique():
            ...
            avg_ranks[str(profile_id)] = float(np.mean(profile_ranks))
        
        return avg_ranks

    def get_top_profiles(self, n: int = 5) -> List[Dict]:
        """Get top N profiles by total utility"""
        ...
        # Sort by utility and return top N
    ...
    def get_results(self):
        ...
        rank_distributions = self.calculate_rank_distribution()
        average_ranks = self.calculate_average_ranks()
...
def main():
    try:
        ...
        # Add market simulation for default scenario
        # Create 3 example products for simulation
        example_products = []
        for i in range(min(3, len(analyzer.get_top_profiles(3)))):
            profile = analyzer.get_top_profiles(3)[i]
            example_products.append(profile['configuration'])
