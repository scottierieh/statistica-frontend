...
class NonParametricTests:
    ...
    def mann_whitney_u_test(self, group_col: str, value_col: str, groups: List[str] = None, alternative: str = 'two-sided', alpha: float = 0.05) -> Dict:
        ...
        # Add min and max to descriptive stats
        desc_stats = {
            groups[0]: {
                'count': n1,  # Changed from 'n' to 'count'
                'median': float(np.median(group1_data)), 
                'mean': float(np.mean(group1_data)),
                'std': float(np.std(group1_data, ddof=1)) if n1 > 1 else 0,
                'min': float(np.min(group1_data)) if n1 > 0 else None,
                'max': float(np.max(group1_data)) if n1 > 0 else None,
                'mean_rank': R1 / n1 if n1 > 0 else 0
            },
            groups[1]: {
                'count': n2,  # Changed from 'n' to 'count'
                'median': float(np.median(group2_data)), 
                'mean': float(np.mean(group2_data)),
                'std': float(np.std(group2_data, ddof=1)) if n2 > 1 else 0,
                'min': float(np.min(group2_data)) if n2 > 0 else None,
                'max': float(np.max(group2_data)) if n2 > 0 else None,
        ...
        result = {
            ...
            'groups': groups,
            'group_stats': desc_stats,  # Changed from 'descriptive_stats' to 'group_stats'
    ...
    def kruskal_wallis_test(self, group_col: str, value_col: str, alpha: float = 0.05) -> Dict:
        ...
        # Calculate descriptive statistics for each group (add min/max)
        group_names = self.data[group_col].unique()
        desc_stats = {}
        for i, group_name in enumerate(group_names):
            group_data = groups_data[i]
            desc_stats[group_name] = {
                'count': len(group_data),  # Changed from 'n' to 'count'
                ...
                'min': float(np.min(group_data)) if len(group_data) > 0 else None,
                'max': float(np.max(group_data)) if len(group_data) > 0 else None
            }
    ...
    def friedman_test(self, variables: List[str], alpha: float = 0.05) -> Dict:
        ...
        # Calculate descriptive statistics for each variable (add min/max)
        desc_stats = {}
        for i, var in enumerate(variables):
            var_data = data_matrix[:, i]
            desc_stats[var] = {
                'count': len(var_data),  # Changed from 'n' to 'count'
                'median': float(np.median(var_data)),
                'mean': float(np.mean(var_data)),
                'std': float(np.std(var_data, ddof=1)) if len(var_data) > 1 else 0,
                'min': float(np.min(var_data)) if len(var_data) > 0 else None,
                'max': float(np.max(var_data)) if len(var_data) > 0 else None
            }
        ...
        result = {
            ...
            'alpha': alpha, 
            'is_significant': is_significant,
            'condition_stats': desc_stats,  # Changed from 'descriptive_stats' to 'condition_stats'
    ...
    def _interpret_mann_whitney(self, is_sig, groups, p, group_col, value_col, desc_stats, z_score, alpha):
        ...
        return {'conclusion': interpretation.strip()}
    ...
    def plot_results(self, test_type):
        ...
        if test_type == 'mcnemar':
            ...
            # Create 1x3 subplot layout for Mann-Whitney
            ...
            for i, group in enumerate(groups):
                group_data = self.data[self.data[group_col] == group][value_col].dropna()
                axes[2].hist(group_data, alpha=0.6, label=group, bins=20, 
                           color=sns.color_palette('crest', n_colors=2)[i])
            axes[2].set_title('Overlapping Histograms', fontsize=12, fontweight='bold')
            axes[2].set_xlabel(value_col, fontsize=11)
            axes[2].set_ylabel('Frequency', fontsize=11)
            axes[2].legend(title=group_col, loc='best')
            ...
            # Keep 2x2 layout for other tests
            fig, axes = plt.subplots(2, 2, figsize=(15, 12))
            
            if test_type == 'wilcoxon':
                # Create 1x3 subplot layout for Wilcoxon
                vars = result['variables']
                
                fig, axes = plt.subplots(1, 3, figsize=(32, 10))
                
                # Paired box plots
                sns.boxplot(data=self.data[vars], palette='crest', ax=axes[0])
                axes[0].set_title('Paired Variables Distribution', fontsize=12, fontweight='bold')
                axes[0].set_ylabel('Values', fontsize=11)
                axes[0].set_xlabel('Variables', fontsize=11)
                
                # Distribution of differences
                differences = self.data[vars[0]] - self.data[vars[1]]
                sns.histplot(differences.dropna(), kde=True, color='#5B9BD5', ax=axes[1])
                axes[1].axvline(x=0, color='red', linestyle='--', alpha=0.7)
                axes[1].set_title('Distribution of Differences', fontsize=12, fontweight='bold')
                axes[1].set_xlabel(f'{vars[0]} - {vars[1]}', fontsize=11)
                axes[1].set_ylabel('Frequency', fontsize=11)
                
                # Scatter plot of paired observations
                axes[2].scatter(self.data[vars[0]], self.data[vars[1]], alpha=0.6, color='#5B9BD5')
                ...
                axes[2].set_title('Paired Observations', fontsize=12, fontweight='bold')
                axes[2].set_xlabel(vars[0], fontsize=11)
                axes[2].set_ylabel(vars[1], fontsize=11)
                ...
                # Create 1x3 subplot layout for Kruskal-Wallis
                group_col = result['group_col']
                value_col = result['value_col']
                ...
                sns.boxplot(x=group_col, y=value_col, data=self.data, 
                          palette='crest', ax=axes[0])
                axes[0].set_title('Box Plot by Group', fontsize=12, fontweight='bold')
                axes[0].set_xlabel(group_col, fontsize=11)
                axes[0].set_ylabel(value_col, fontsize=11)
                if len(result['group_stats']) > 5:
                    axes[0].tick_params(axis='x', rotation=45)
                
                # Violin plot
                sns.violinplot(x=group_col, y=value_col, data=self.data, 
                             palette='crest', ax=axes[1])
                axes[1].set_title('Violin Plot by Group', fontsize=12, fontweight='bold')
                axes[1].set_xlabel(group_col, fontsize=11)
                axes[1].set_ylabel(value_col, fontsize=11)
                if len(result['group_stats']) > 5:
                ...
                # Mean ranks bar plot
                groups = list(result['group_stats'].keys())
                mean_ranks = []
                for g in groups:
                ...
                axes[2].bar(range(len(groups)), mean_ranks, color=colors, alpha=0.7, edgecolor='black')
                axes[2].set_title('Mean Ranks by Group', fontsize=12, fontweight='bold')
                axes[2].set_xlabel(group_col, fontsize=11)
                axes[2].set_ylabel('Mean Rank', fontsize=11)
                axes[2].set_xticks(range(len(groups)))
                axes[2].set_xticklabels(groups, rotation=45 if len(groups) > 5 else 0)
                ...
                # Create 1x3 subplot layout for Friedman
                vars = result['variables']
                ...
                axes[2].bar(range(len(vars)), mean_ranks, color=colors, alpha=0.7, edgecolor='black')
                axes[2].set_title('Mean Ranks by Condition', fontsize=12, fontweight='bold')
                axes[2].set_xlabel('Condition', fontsize=11)
                axes[2].set_ylabel('Mean Rank', fontsize=11)
                axes[2].set_xticks(range(len(vars)))
                axes[2].set_xticklabels(vars, rotation=45 if len(vars) > 5 else 0)
            ...
            # Add overall title with test results (only for tests not using 1x3 layout)
            if test_type not in ['mann_whitney', 'wilcoxon', 'kruskal_wallis', 'friedman']:
                p_val = result['p_value']
                p_text = f"p < 0.001" if p_val < 0.001 else f"p = {p_val:.3f}"
                sig_text = "Significant" if result['is_significant'] else "Not Significant"
             

        # Adjust layout based on test type
        if test_type in ['mann_whitney', 'wilcoxon', 'kruskal_wallis', 'friedman']:
