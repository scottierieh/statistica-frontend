...
class RMSPropOptimizer:
    def __init__(self, objective_function, bounds, learning_rate=0.01, decay_rate=0.9, epsilon=1e-8, max_iter=1000):
        ...
        self.cache = np.zeros(self.n_dims)
    ...
    def run(self):
        for i in range(self.max_iter):
            ...
            self.current_solution -= self.learning_rate * grad / (np.sqrt(self.cache) + self.epsilon)
