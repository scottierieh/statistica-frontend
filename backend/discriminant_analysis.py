...
def main():
    try:
        ...
        try:
            ...
            lda_metrics = _calculate_metrics(y_encoded, y_pred_lda)
            
            group_means_lda = []
            for i, group in enumerate(le.classes_):
                group_means_lda.append(X_scaled[df_clean[group_var] == group].mean(axis=0))
        ...
        try:
            ...
            qda_metrics = _calculate_metrics(y_encoded, y_pred_qda)
            
            group_means_qda = []
            for i, group in enumerate(le.classes_):
                 group_means_qda.append(X_scaled[df_clean[group_var] == group].mean(axis=0))
