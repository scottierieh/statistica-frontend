...
def create_box_plot(df, x_col, y_col, title):
    ...
    for category in df[x_col].unique():
        fig.add_trace(go.Box(y=df[df[x_col] == category][y_col], name=category))
...
def create_pie_chart(df, labels_col, values_col, title, hole=0):
    ...
    fig = go.Figure(data=[go.Pie(labels=counts.index, values=counts.values, hole=hole)])
    ...
    try:
        ...
        try:
            ...
            if all(c and c in df.columns for c in [user_id_col, cohort_date_col, date_col]):
                ...
                fig = go.Figure(data=go.Heatmap(z=retention_matrix.values, x=retention_matrix.columns.astype(str), y=retention_matrix.index.astype(str), colorscale='Viridis', text=np.around(retention_matrix.values, 2), texttemplate="%{text}%"))
            ...
            if all(c and c in df.columns for c in [item_category_col, item_brand_col]):
                ...
                fig = go.Figure(data=go.Heatmap(z=crosstab.values, x=crosstab.columns, y=crosstab.index, colorscale='Blues'))
