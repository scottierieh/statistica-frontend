...
def main():
    try:
        ...
        numeric_cols = [
            ...
            'pageViewsCol', 'sessionDurationCol'
        ]
        date_cols = ['dateCol', 'cohortDateCol']
        ...
        try:
            ...
            if all(c and c in df.columns for c in [campaign_col, cost_col, revenue_col]):
                ...
                fig = create_bar_chart(campaign_stats, x='roi', y=campaign_col, title='Campaign ROI (%)', xlabel='ROI (%)', ylabel='Campaign', orientation='h')
        ...
        try:
            ...
            if conversion_col and conversion_col in df.columns:
                ...
                funnel_df = pd.DataFrame({'Stage': ['Total Sessions', 'Converted'], 'Count': [total_sessions, converted_sessions]})
        ...
        try:
            ...
            if all(c and c in df.columns for c in [source_col, conversion_col, revenue_col]):
                ...
                if not converted_df.empty:
                    attribution = converted_df.groupby(source_col)[revenue_col].agg(['sum', 'count']).reset_index().rename(columns={'sum': 'TotalRevenue', 'count': 'Conversions'}).sort_values('TotalRevenue', ascending=False)
                    fig = create_bar_chart(attribution, x='TotalRevenue', y=source_col, title='Attributed Revenue (Last Touch)', xlabel='Attributed Revenue', ylabel='Source', orientation='h')
                    fig.update_layout(yaxis={'categoryorder':'total ascending'})
        ...
        try:
            ...
            if all(c and c in df.columns for c in [user_id_col, cohort_date_col, date_col]):
                ...
                fig = go.Figure(data=go.Heatmap(z=retention_matrix.values, x=retention_matrix.columns.astype(str), y=retention_matrix.index.astype(str), hoverongaps=False, colorscale='Viridis', text=np.around(retention_matrix.values, 2), texttemplate="%{text}%"))
        ...
        try:
            ...
            if all(c and c in df.columns for c in [item_category_col, item_brand_col]):
                ...
                fig = go.Figure(data=go.Heatmap(z=crosstab.values, x=crosstab.columns, y=crosstab.index, hoverongaps=False, colorscale='Blues'))
        ...
        try:
            ...
            if all(c and c in df.columns for c in [price_col, quantity_col]):
                fig = go.Figure(data=go.Scatter(x=df[price_col], y=df[quantity_col], mode='markers'))
        ...
        try:
            ...
            if all(c and c in df.columns for c in [age_col, gender_col]):
                ...
                fig = go.Figure(go.Treemap(
                    labels=segment_counts[age_col] + ' - ' + segment_counts[gender_col],
                    parents=[""] * len(segment_counts),
                    values=segment_counts['count'],
                    textinfo="label+value+percent root"
                ))
                fig.update_layout(title='Customer Segments by Age & Gender', margin=dict(t=50, l=25, r=25, b=25))
                plots['segmentationTreemap'] = to_json(fig)
        except Exception:
            plots['segmentationTreemap'] = None
        ...
        try:
            ...
            plots['geographicDistribution'] = None
        ...
        try:
            ...
            if all(c and c in df.columns for c in [ltv_col, membership_col]):
                fig = go.Figure()
                for level in df[membership_col].unique():
                    fig.add_trace(go.Box(y=df[df[membership_col] == level][ltv_col], name=level))
                fig.update_layout(title='LTV by Membership Level', yaxis_title='Customer Lifetime Value (LTV)')
                plots['ltvByMembership'] = to_json(fig)
        except Exception:
            plots['ltvByMembership'] = None

        try:
            device_col = config.get('deviceCol')
            if device_col and device_col in df.columns:
                device_counts = df[device_col].value_counts()
                fig = go.Figure(data=[go.Pie(labels=device_counts.index, values=device_counts.values, hole=.4)])
                fig.update_layout(title_text='Sessions by Device')
                plots['deviceUsage'] = to_json(fig)
        except Exception:
            plots['deviceUsage'] = None
