
import sys
import json
import numpy as np
import pandas as pd
from linearmodels.panel import PanelOLS, RandomEffects
import statsmodels.api as sm
import warnings

warnings.filterwarnings('ignore')

def _to_native_type(obj):
    if isinstance(obj, np.integer): return int(obj)
    if isinstance(obj, (np.floating, float)): return float(obj) if np.isfinite(obj) else None
    if isinstance(obj, np.ndarray): return obj.tolist()
    if isinstance(obj, (bool, np.bool_)): return bool(obj)
    return str(obj)

def generate_interpretation(fe_result, re_result, hausman_result):
    """Generates an interpretation for the panel data regression results."""
    interp = "### Panel Data Regression Interpretation\n\n"
    
    # Hausman Test Interpretation
    hausman_p = hausman_result.get('p_value')
    if hausman_p is not None:
        if hausman_p < 0.05:
            interp += f"The **Hausman test** was significant (p = {hausman_p:.4f}), suggesting that the **Fixed Effects (FE) model is more appropriate** than the Random Effects (RE) model. This indicates that unobserved entity-specific effects are likely correlated with the model's predictors.\n\n"
            preferred_model = fe_result
        else:
            interp += f"The **Hausman test** was not significant (p = {hausman_p:.4f}), suggesting that the **Random Effects (RE) model is more efficient** and appropriate. This implies that the unobserved entity-specific effects are likely not correlated with the predictors.\n\n"
            preferred_model = re_result
    else:
        interp += "The Hausman test could not be performed, so a definitive choice between FE and RE models cannot be made statistically. We will interpret the FE model by default as it is generally more robust to omitted variable bias.\n\n"
        preferred_model = fe_result
        
    # Interpretation of the preferred model
    interp += f"**Interpreting the {preferred_model.name} Model:**\n"
    
    significant_vars = []
    for param, pval in preferred_model.pvalues.items():
        if param != 'Intercept' and pval < 0.05:
            coef = preferred_model.params[param]
            direction = "positively" if coef > 0 else "negatively"
            significant_vars.append(f"**{param}** (coef: {coef:.3f})")

    if significant_vars:
        interp += f"- The model shows that {', '.join(significant_vars)} significantly influence(s) the dependent variable, holding other factors constant.\n"
    else:
        interp += "- None of the independent variables showed a statistically significant effect on the dependent variable in this model.\n"
        
    rsquared_key = 'rsquared_within' if 'rsquared_within' in preferred_model else 'rsquared'
    if rsquared_key in preferred_model:
        r2 = preferred_model[rsquared_key]
        interp += f"- The model explains approximately **{(r2 * 100):.1f}%** of the variance in the dependent variable ({'within entities' if rsquared_key == 'rsquared_within' else 'overall'}).\n"

    interp += "\n**Conclusion:** Based on the analysis, the chosen model provides insights into the relationships while accounting for panel data structure. This approach is more reliable than a simple Pooled OLS regression which ignores entity-specific effects."
    
    return interp


def main():
    try:
        payload = json.load(sys.stdin)
        data = pd.DataFrame(payload.get('data'))
        dependent = payload.get('dependent')
        exog = payload.get('exog')
        entity_col = payload.get('entity_col')
        time_col = payload.get('time_col')

        if not all([dependent, exog, entity_col, time_col]):
            raise ValueError("Missing required parameters.")
            
        df = data.copy()
        
        # Ensure correct types
        for col in [dependent] + exog:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        df = df.dropna(subset=[dependent] + exog + [entity_col, time_col])
        
        # Set panel data index
        df[time_col] = pd.to_datetime(df[time_col])
        df = df.set_index([entity_col, time_col])
        
        y = df[dependent]
        X = sm.add_constant(df[exog])

        # --- Pooled OLS ---
        pooled_ols = PanelOLS(y, X).fit(cov_type='robust')
        
        # --- Fixed Effects ---
        fe_model = PanelOLS(y, X, entity_effects=True).fit(cov_type='robust')
        
        # --- Random Effects ---
        re_model = RandomEffects(y, X).fit(cov_type='robust')

        # --- F-test for entity effects ---
        f_test_entity = fe_model.f_statistic_robust
        
        # --- Hausman Test ---
        hausman_results = None
        try:
            hausman_test = fe_model.compare(re_model)
            hausman_results = {
                'statistic': hausman_test.stat,
                'p_value': hausman_test.pval,
                'df': hausman_test.dof,
                'interpretation': 'Fixed Effects Recommended' if hausman_test.pval < 0.05 else 'Random Effects Recommended'
            }
        except Exception as e:
            hausman_results = {'error': str(e), 'interpretation': 'Test failed.'}
            
        def format_summary(model_fit, name):
            summary_dict = {
                'name': name,
                'params': model_fit.params.to_dict(),
                'pvalues': model_fit.pvalues.to_dict(),
                'tstats': model_fit.tstats.to_dict(),
                'rsquared': getattr(model_fit, 'rsquared_overall', getattr(model_fit, 'rsquared', None)),
                'rsquared_within': getattr(model_fit, 'rsquared_within', None),
            }
            return summary_dict

        pooled_res = format_summary(pooled_ols, 'Pooled OLS')
        fe_res = format_summary(fe_model, 'Fixed Effects')
        re_res = format_summary(re_model, 'Random Effects')
        
        interpretation = generate_interpretation(fe_res, re_res, hausman_results)

        response = {
            'results': {
                'pooled_ols': pooled_res,
                'fixed_effects': fe_res,
                'random_effects': re_res,
                'f_test_entity': {
                    'statistic': f_test_entity.stat,
                    'p_value': f_test_entity.pval,
                    'df': f_test_entity.dof,
                    'df_resid': f_test_entity.dof_resid,
                    'interpretation': 'Fixed effects are significant (use FE over OLS)' if f_test_entity.pval < 0.05 else 'Fixed effects are not significant (OLS may be sufficient)'
                },
                'hausman_test': hausman_results,
                'interpretation': interpretation
            }
        }
        
        print(json.dumps(response, default=_to_native_type, ensure_ascii=False))

    except Exception as e:
        print(json.dumps({"error": str(e)}), file=sys.stderr)
        sys.exit(1)

if __name__ == '__main__':
    main()
