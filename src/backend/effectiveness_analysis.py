...
class EffectivenessAnalyzer:
    ...
    def _prepare_data(self):
        ...
        if self.time_var:
            ...
            if len(self.time_values) != 2:
                self.time_var = None # Disable time-based analysis if not binary
    ...
    def analyze_pre_post(self):
        ...
        pre_data = self.df_clean[self.df_clean[self.time_var] == pre_label][self.outcome_var]
        post_data = self.df_clean[self.df_clean[self.time_var] == post_label][self.outcome_var]
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        sns.pointplot(x=self.time_var, y=self.outcome_var, data=self.df_clean, ax=ax)
    ...
    def analyze_did(self):
        if not self.time_var or not self.group_var: return {"error": "DID requires time and group variables."}

        df_did = self.df_clean.copy()
        df_did['treat'] = (df_did[self.group_var] == self.group_values[1]).astype(int)
        df_did['post'] = (df_did[self.time_var] == self.time_values[1]).astype(int)
        ...
        ax.legend(handles, [str(self.group_values[0]), str(self.group_values[1])], title=self.group_var)
    ...
    def analyze_sensitivity(self):
        if not self.time_var or not self.group_var: return None
