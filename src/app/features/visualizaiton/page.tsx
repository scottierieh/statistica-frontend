...
import { 
  ...
  BarChart as BarChartIcon, PieChart as PieChartIcon, Box, Dot, Heater, HelpCircle, 
  MoveRight, Settings, FileSearch, Play, ArrowLeft, ChevronLeft, ChevronRight,
  Eye, EyeOff, Download, Sparkles, RefreshCw, Layers, TrendingUp, Grid3X3,
  Palette, Maximize2, Share2, Upload, FileSpreadsheet, Loader2, X, Database,
...
// IntroPage Component
const IntroPage = ({ onFileUpload, onSampleData, isLoading }: { 
  ...
  return (
    <div 
      className="flex flex-1 items-center justify-center p-4 bg-gradient-to-br from-slate-50 via-white to-slate-100 min-h-screen"
      onDragOver={e => { e.preventDefault(); setIsDragOver(true); }}
      onDragLeave={e => { e.preventDefault(); setIsDragOver(false); }}
      onDrop={e => { e.preventDefault(); setIsDragOver(false); const files = Array.from(e.dataTransfer.files); if (files.length > 0) onFileUpload(files); }}
    >
      <AnimatePresence>
        {isDragOver && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-primary/10 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white rounded-2xl shadow-2xl p-12 text-center border-2 border-dashed border-primary">
              ...
              <p className="text-muted-foreground">CSV, Excel, or JSON</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Card className="w-full max-w-4xl shadow-2xl border-0 overflow-hidden">
          <CardHeader className="text-center p-10 bg-gradient-to-r from-primary via-primary/90 to-primary/80 text-white">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mx-auto mb-4">
              <BarChartIcon className="w-8 h-8" />
            </div>
            <CardTitle className="font-headline text-4xl font-bold">Data Visualization</CardTitle>
            <CardDescription className="text-xl pt-2 text-white/80">Create stunning charts and graphs</CardDescription>
          </CardHeader>
          <CardContent className="space-y-8 px-8 py-10">
            <div className="text-center space-y-4">
              <h2 className="text-2xl font-semibold text-primary">Get Started</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto">Upload your data or use sample data</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-4">
                <input type="file" ref={fileInputRef} onChange={e => { const files = Array.from(e.target.files || []); if (files.length > 0) onFileUpload(files); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="hidden" accept=".csv,.txt,.tsv,.xlsx,.xls,.json" />
                <Button size="lg" onClick={() => fileInputRef.current?.click()} disabled={isLoading} className="shadow-lg min-w-[200px]">
                  {isLoading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Upload className="w-5 h-5 mr-2" />}Upload File
                </Button>
                <span className="text-muted-foreground">or</span>
                <Button size="lg" variant="outline" onClick={onSampleData} disabled={isLoading} className="min-w-[200px]"><Sparkles className="w-5 h-5 mr-2" />Sample Data</Button>
              </div>
              <p className="text-xs text-muted-foreground pt-2">CSV, Excel (.xlsx, .xls), JSON</p>
            </div>
            <Separator />
            <div className="grid md:grid-cols-2 gap-6 text-sm">
              <div><h3 className="font-semibold mb-3 flex items-center gap-2"><Settings className="w-4 h-4 text-primary" />How It Works</h3>
                <ol className="space-y-2 text-muted-foreground">
                  <li>1. Upload your data file</li><li>2. Select chart category & type</li><li>3. Configure variables</li><li>4. Generate visualization</li>
                </ol>
              </div>
              <div><h3 className="font-semibold mb-3 flex items-center gap-2"><FileSearch className="w-4 h-4 text-primary" />40+ Chart Types</h3>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• Distribution: Histogram, Box, Violin, Q-Q</li>
                  <li>• Relationship: Scatter, Line, Heatmap</li>
                  <li>• Categorical: Bar, Pie, Treemap, Sankey</li>
                  <li>• Advanced: PCA, Cluster, Network</li>
                </ul>
              </div>
            </div>
          </CardContent>
          <CardFooter className="flex justify-center p-6 bg-slate-50 border-t">
            <Button variant="outline" asChild><Link href="/dashboard/analysis"><ArrowLeft className="mr-2 h-4 w-4" />Back to Hub</Link></Button>
          </CardFooter>
        </Card>
      </motion.div>
    </div>
...
// ChartRenderer
const ChartRenderer = ({ plotData, onDownload, onFullscreen }: { plotData: string | null; onDownload?: () => void; onFullscreen?: () => void; }) => {
        ...
        <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div><CardTitle className="text-lg">Generated Chart</CardTitle><p className="text-xs text-muted-foreground">Export or expand</p></div>
            </div>
            <div className="flex gap-2">
              <TooltipProvider><Tooltip><TooltipTrigger asChild><Button variant="outline" size="icon" onClick={onDownload}><Download className="w-4 h-4" /></Button></TooltipTrigger><TooltipContent>Download</TooltipContent></Tooltip></TooltipProvider>
              <TooltipProvider><Tooltip><TooltipTrigger asChild><Button variant="outline" size="icon" onClick={onFullscreen}><Maximize2 className="w-4 h-4" /></Button></TooltipTrigger><TooltipContent>Fullscreen</TooltipContent></Tooltip></TooltipProvider>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-6 bg-white">
          <div className="w-full flex items-center justify-center rounded-xl overflow-hidden bg-slate-50 p-4">
            <Image src={plotData} alt="Chart" width={1000} height={800} className="max-w-full h-auto" />
          </div>
        </CardContent>
      </Card>
    </motion.div>
...
export default function VisualizationPage() {
  ...
  useEffect(() => {
    if (scatterY === scatterX && numericHeaders.length > 1) {
  ...
  const handleRunAnalysis = useCallback(async (chartType: string) => {
    ...
    try {
      ...
      if (!result?.plot) throw new Error('Chart not returned');
  ...
  const handleDownload = useCallback(() => {
    if (!analysisResult?.plot) return;
    const link = document.createElement('a');
    link.href = analysisResult.plot;
    link.download = `chart-${activeChart}-${Date.now()}.png`;
    link.click();
    toast({ title: '✓ Downloaded' });
  }, [analysisResult, activeChart, toast]);

  const clearData = useCallback(() => { setData([]); setFileName(''); setAnalysisResult(null); setActiveChart(null); setView('intro'); }, []);

  if (view === 'intro') return <IntroPage onFileUpload={processFiles} onSampleData={loadSampleData} isLoading={isFileLoading} />;

  const wrap = <T extends (...args: any[]) => any>(fn: T) => (...args: Parameters<T>) => { markDirty(); return fn(...args); };
  const numLen = numericHeaders.length, catLen = categoricalHeaders.length, allLen = allHeaders.length;
          ...
          <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 320, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="border-r border-slate-200 bg-white shadow-lg flex flex-col overflow-hidden">
            <div className="p-5 border-b bg-gradient-to-r from-primary to-primary/80">
              <div className="flex items-center gap-3 text-white">
                <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center"><BarChartIcon className="w-5 h-5" /></div>
                <div><h2 className="font-semibold text-lg">Visualization</h2><p className="text-xs text-primary-foreground/70">Chart Builder</p></div>
              </div>
            </div>
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-4">
                ...
                <Card className="border-slate-200">
                  <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><FileSpreadsheet className="w-4 h-4 text-primary" />Data</CardTitle></CardHeader>
                  <CardContent className="space-y-3">
                    {canRun ? (
                      <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2"><Database className="w-4 h-4 text-green-600" /><span className="text-sm font-medium text-green-800 truncate max-w-[140px]">{fileName}</span></div>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={clearData}><X className="w-3 h-3" /></Button>
                        </div>
                        <p className="text-xs text-green-600 mt-1">{data.length} rows × {allLen} cols | {numLen} num, {catLen} cat</p>
                      </div>
                    ) : (
                      <div className="p-3 bg-amber-50 rounded-lg border border-amber-200"><p className="text-sm text-amber-800">No data</p></div>
                    )}
                    ...
                    <Button onClick={() => fileInputRef.current?.click()} className="w-full" size="sm" disabled={isFileLoading}>{isFileLoading ? <Loader2 className="mr-2 w-4 h-4 animate-spin" /> : <Upload className="mr-2 w-4 h-4" />}Upload</Button>
                    <Button variant="outline" onClick={loadSampleData} className="w-full" size="sm"><Sparkles className="mr-2 w-4 h-4" />Sample</Button>
                  </CardContent>
                </Card>
                ...
                <Card className="border-slate-200">
                  <CardContent className="pt-4 space-y-2">
                    ...
                    <Button variant="outline" onClick={() => setView('intro')} className="w-full"><HelpCircle className="w-4 h-4 mr-2" />Help</Button>
                  </CardContent>
                </Card>
              </div>
            </ScrollArea>
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div ref={resultRef}>
            {isLoading ? (
              ...
              <ChartRenderer plotData={analysisResult.plot} onDownload={handleDownload} onFullscreen={() => analysisResult?.plot && window.open(analysisResult.plot, '_blank')} />
            ) : !canRun ? (
