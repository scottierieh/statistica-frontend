...
export async function POST(req: NextRequest) {
  try {
    ...
    const scriptPath = path.resolve(process.cwd(), 'backend', 'demand_forecasting_analysis.py');
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          ...
          try {
            ...
            console.error('Failed to parse python script output for demand forecasting');
