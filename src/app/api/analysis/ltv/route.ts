...
export async function POST(req: NextRequest) {
  try {
    ...
    const scriptPath = path.resolve(process.cwd(), 'backend', 'ltv_analysis.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`LTV Script exited with code ${code}`);
          console.error(error);
          try {
          ...
          try {
            ...
            console.error('Failed to parse python script output for LTV');
