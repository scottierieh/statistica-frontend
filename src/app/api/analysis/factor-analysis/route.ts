...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.data || !body.return_col || !body.market_col) {
      return NextResponse.json(
        { error: 'Missing required fields: data, return_col, market_col' },
        { status: 400 }
      );
    }

    const pythonExecutable = path.resolve(process.cwd(), 'backend', 'venv', 'bin', 'python');
    const scriptPath = path.resolve(process.cwd(), 'backend', 'factor_analysis.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`Factor Analysis script exited with code ${code}`);
