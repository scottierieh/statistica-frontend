...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.data || !body.date_col || !body.price_col) {
      return NextResponse.json(
        { error: 'Missing required fields: data, date_col, price_col' },
        { status: 400 }
      );
    }

    const pythonExecutable = path.resolve(process.cwd(), 'backend', 'venv', 'bin', 'python');
    const scriptPath = path.resolve(process.cwd(), 'backend', 'backtesting.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`Backtesting script exited with code ${code}`);
