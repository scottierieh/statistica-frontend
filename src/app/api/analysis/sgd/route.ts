...
export async function POST(req: NextRequest) {
  try {
    ...
    const scriptPath = path.resolve(process.cwd(), 'backend', 'sgd_simulation.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`SGD Script exited with code ${code}: ${error}`);
          try {
          ...
          try {
            ...
            console.error('Failed to parse SGD script output:', result);
