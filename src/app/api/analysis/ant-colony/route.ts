...
export async function POST(req: NextRequest) {
  try {
    ...
    const scriptPath = path.resolve(process.cwd(), 'backend', 'ant_colony_analysis.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`ACO Script exited with code ${code}`);
          console.error(error);
          try {
          ...
          try {
            ...
            console.error('Failed to parse ACO script output:', result);
