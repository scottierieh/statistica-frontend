...
export async function POST(req: NextRequest) {
  try {
    ...
    const scriptPath = path.resolve(process.cwd(), 'backend', 'panel_data_regression_analysis.py');
    ...
    return new Promise<NextResponse>((resolve) => {
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`Panel Data Script exited with code ${code}`);
          console.error(error);
          try {
          ...
          try {
            ...
            console.error('Failed to parse panel data script output');
