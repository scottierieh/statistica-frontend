...
import {
  ...
  ArrowRight, LayoutDashboard, Database, FlaskConical
} from "lucide-react";
  ...
  const destinations = [
    {
      ...
      description: 'Live charts, signals, and valuation analysis',
      path: '/dashboard/finance-dashboard',
      iconBg: 'border-primary/20 bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground',
    },
    {
      icon: Database,
      label: 'Shared Data',
      description: 'View and download saved CSV files',
      path: '/dashboard/shared-data',
      iconBg: 'border-primary/20 bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground',
    },
    {
      icon: FlaskConical,
      label: 'Finance Analytics',
      description: 'Statistical analysis, correlation, and modeling',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-4 animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Success banner */}
        <div className="flex items-center gap-3 px-5 py-3.5 rounded-t-2xl bg-primary text-primary-foreground">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/20">
            <CheckCircle2 className="w-4 h-4" />
          </div>
          <div className="flex-1">
            <p className="text-sm font-bold">Data Sync Complete</p>
            <p className="text-xs text-primary-foreground/70">{tickers.length} tickers · {analysisCount} analysis types</p>
          </div>
        ...
        <div className="bg-card border border-t-0 border-border rounded-b-2xl shadow-xl overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-border bg-muted/30">
            <div className="flex flex-wrap gap-1.5">
              {tickers.slice(0, 9).map(t => (
                <span key={t} className="px-2 py-0.5 rounded-full bg-muted border border-border text-[10px] font-mono font-semibold text-foreground">
                  {t}
                </span>
              ))}
              {tickers.length > 9 && (
                <span className="px-2 py-0.5 rounded-full bg-muted border border-border text-[10px] font-mono text-muted-foreground">
                  +{tickers.length - 9} more
                </span>
              )}
            </div>
          </div>
          ...
          <div className="p-4 space-y-2">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-1 mb-3">
            ...
            {destinations.map(dest => (
              <button
                ...
                onClick={() => {
                  ...
                  window.open(`${dest.path}?${params.toString()}`, '_blank');
                }}
                className={cn(
                  "group relative w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card text-left transition-all duration-300",
                  "hover:border-primary/50 hover:shadow-md hover:shadow-primary/5 hover:ring-1 hover:ring-primary/50"
                )}
              >
                ...
                <div className={cn(
                  "relative flex h-11 w-11 items-center justify-center rounded-2xl border transition-all duration-300 group-hover:scale-110",
                  dest.iconBg
                )}>
                  <dest.icon className="w-5 h-5" />
                </div>
                <div className="relative flex-1 min-w-0">
                  <div className="text-sm font-bold text-foreground">{dest.label}</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{dest.description}</div>
                </div>
          ...
          <div className="px-4 pb-4">
            <button
              onClick={onClose}
              className="w-full py-2.5 rounded-xl text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-all"
            >
              Stay on this page
            </button>
          </div>
...
export default function FinanceApiPage({ title }: { title?: string }) {
  ...
  const handleSync = async () => {
    ...
    setIsSyncing(true);
    try {
      const result = await triggerFinanceSync({ orgId: TEMP_ORG_ID, tickers, analysisTypes: selectedAnalysis, period });
    ...
    <>
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex flex-col gap-2">
          <h2 className="text-2xl font-bold tracking-tight">Finance API</h2>
          <p className="text-muted-foreground">Connect to Yahoo Finance to fetch stock prices, fundamentals, and market data.</p>
        </div>
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm">Select Tickers</h3>
                <p className="text-xs text-muted-foreground">Choose a preset universe or add custom tickers</p>
              </div>
            </div>
            <div className="flex gap-3">
              <div className="flex-1">
                <Label className="text-xs mb-1 block">Preset Universe</Label>
              ...
              <div className="flex-1">
                <Label className="text-xs mb-1 block">Data Period</Label>
                <Select value={period} onValueChange={setPeriod}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {[['1mo','1 Month'],['3mo','3 Months'],['6mo','6 Months'],['1y','1 Year'],['2y','2 Years'],['5y','5 Years'],['max','Max']].map(([v,l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="flex gap-2">
              <Input placeholder="Enter ticker (e.g. AAPL, 005930.KS)" value={tickerInput} onChange={e => setTickerInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTicker()} className="flex-1" />
              <Button size="sm" onClick={addTicker} disabled={!tickerInput.trim()}><Plus className="w-4 h-4 mr-1" /> Add</Button>
            </div>
            {tickers.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tickers.map(t => (
                  <Badge key={t} variant="secondary" className="px-2 py-1 text-xs font-mono gap-1">
                    {t}
                    <button onClick={() => removeTicker(t)} className="ml-1 hover:text-destructive"><X className="w-3 h-3" /></button>
                  </Badge>
                ))}
                <span className="text-[10px] text-muted-foreground self-center ml-2">{tickers.length} ticker{tickers.length > 1 ? 's' : ''}</span>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-sm">Select Analysis Types</h3>
                <p className="text-xs text-muted-foreground">Variables are auto-mapped based on selection.</p>
              </div>
              <button
                onClick={() => setSelectedAnalysis(
                  selectedAnalysis.length === analysisTypes.length ? [] : analysisTypes.map(a => a.id)
                )}
                ...
                return (
                  <button key={a.id} onClick={() => toggleAnalysis(a.id)} className={cn("flex flex-col items-start p-4 rounded-xl border-2 text-left transition-all", isSelected ? "border-primary bg-primary/5 shadow-sm" : "border-muted hover:border-muted-foreground/30")}>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {a.variables.slice(0, 4).map(v => <Badge key={v} variant="outline" className="text-[8px] px-1 h-4 font-mono">{v}</Badge>)}
                      {a.variables.length > 4 && <Badge variant="outline" className="text-[8px] px-1 h-4">+{a.variables.length - 4}</Badge>}
                    </div>
                  </button>
                );
              })}
        ...
        {/* Sync Button */}
        <Button className="w-full" size="lg" onClick={handleSync} disabled={isSyncing || tickers.length === 0 || selectedAnalysis.length === 0}>
        ...
        {syncHistory.length > 0 && (
          <Card>
            <CardContent className="p-6 space-y-3">
              <h3 className="font-bold text-sm flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-500" />Recent Syncs</h3>
              {syncHistory.map(sync => (
                <div key={sync.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 text-xs cursor-pointer hover:bg-muted/50 transition-colors"
                  ...
                  <div className="flex items-center gap-3">
                    <Badge variant="default" className="bg-emerald-500 text-[9px]">Success</Badge>
        ...
        <Card className="border-muted/60">
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">About Yahoo Finance API</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-muted-foreground">
              <div><p className="font-bold text-foreground mb-1">Free & Open</p><p>No API key required. Stocks, ETFs, crypto, commodities, forex all supported.</p></div>
              <div><p className="font-bold text-foreground mb-1">Auto-Mapped Variables</p><p>All variables are pre-defined per analysis type. No manual column mapping needed.</p></div>
              <div><p className="font-bold text-foreground mb-1">Rate Limits</p><p>Yahoo Finance allows ~2,000 requests/hour. Large universes are batched automatically.</p></div>
            </div>
          </CardContent>
        </Card>
      </div>
