...
import { Loader2, Play, Target, TrendingUp, Activity, Zap, CheckCircle, AlertCircle, Info, Plus, Minus, Bug, Trash2 } from 'lucide-react';
...
export default function AntColonyOptimizationPage() {
    const { toast } = useToast();

    const [objectiveFunction, setObjectiveFunction] = useState('np.sum(x**2)');
    const [nAnts, setNAnts] = useState('50');
    ...
    const [result, setResult] = useState<ACOResult | null>(null);
    ...
    const handleExampleSelect = (example: typeof EXAMPLE_FUNCTIONS[0]) => {
        ...
        setNAnts(example.n_ants);
        setNIterations(example.n_iterations);
        setEvaporationRate(example.evaporation_rate);
    ...
    const handleOptimize = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                evaporation_rate: parseFloat(evaporationRate),
                alpha: parseFloat(alpha),
                beta: parseFloat(beta)
            };

            const response = await fetch(`${FASTAPI_URL}/api/analysis/ant-colony`, {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                throw new Error(errorResult.detail || 'Ant colony optimization failed');
            }

            const res: ACOResult = await response.json();
            setResult(res);

            toast({
                title: "Optimization Complete",
                description: `Best fitness: ${res.best_fitness.toFixed(4)} with ${res.problem.n_ants} ants`
            });
        } catch (e: any) {
        ...
        <div className="max-w-4xl mx-auto space-y-6 p-4">
            ...
            <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                    Bio-inspired optimization mimicking ant foraging behavior with pheromone trails
                </p>
            </div>
            ...
            <Card>
                <CardHeader className="pb-4">
                    <CardTitle className="text-base font-medium">Colony Configuration</CardTitle>
                </CardHeader>
                <CardContent className="space-y-5">
                    {/* Parameters & Examples */}
                    <div className="flex flex-wrap items-end gap-4">
                        <div className="space-y-1.5">
                            <Label className="text-xs text-muted-foreground">Ants</Label>
                            <Input
                                type="number"
                                value={nAnts}
                                onChange={e => setNAnts(e.target.value)}
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Range = Max - Min (larger ranges require more ants for exploration)
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        <Label className="text-sm font-medium">ACO Parameters</Label>
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-xs text-muted-foreground">Evaporation Rate</Label>
                                <Input
                                    type="number"
                    ...
                    <Button onClick={handleOptimize} disabled={isLoading} className="w-full h-10">
                        {isLoading ? (
                            ...
                            <><Play className="mr-2 h-4 w-4" />Start Colony</>
                        )}
                    </Button>
                </CardContent>
            </Card>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        ...
                        <Card className="border-0 shadow-sm">
                            <CardContent className="p-4">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <p className="text-xs text-muted-foreground mb-1">Pheromone</p>
                                        <p className="text-lg font-semibold font-mono">
                                            {result.pheromone_strength.toFixed(2)}
                                        </p>
                                    </div>
                            ...
                            <CardContent>
                                <Tabs defaultValue="convergence" className="w-full">
                                    <TabsList className="w-full justify-start h-9 p-1 bg-muted/50">
                                        ...
                                        {result.plots.pheromone && (
                                            <TabsTrigger value="pheromone" className="text-xs">Pheromone Trails</TabsTrigger>
                                        )}
                                    ...
                                    {result.plots.pheromone && (
                                        <TabsContent value="pheromone" className="mt-4">
                                            <div className="rounded-lg overflow-hidden border">
                                                <Image
                                                    src={`data:image/png;base64,${result.plots.pheromone}`}
                                                    alt="Pheromone Trails"
                                                    width={800}
                                                    height={500}
                                                    className="w-full"
                                                />
                                            ...
                                            <p className="text-xs text-muted-foreground mt-2 text-center">
                                                Evolution of pheromone trail strength over iterations
                                            </p>
                                        </TabsContent>
                                    )}
