...
interface FullAnalysisResponse {
    ols_results: RegressionResult;
    tsls_results: RegressionResult;
}
        ...
        <div className="flex flex-1 items-center justify-center p-4 bg-muted/20">
            <Card className="w-full max-w-4xl shadow-2xl">
                <CardHeader className="text-center p-8 bg-muted/50 rounded-t-lg">
                    ...
                    <CardTitle className="font-headline text-4xl font-bold">Two-Stage Least Squares (2SLS)</CardTitle>
                    <CardDescription className="text-xl pt-2 text-muted-foreground max-w-2xl mx-auto">
                        An instrumental variable method to estimate causal effects in the presence of endogeneity.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-10 px-8 py-10">
                     <div className="text-center">
                        <h2 className="text-2xl font-semibold mb-4">Why Use 2SLS?</h2>
                        <p className="max-w-3xl mx-auto text-muted-foreground">
                           When a predictor variable (X) is correlated with the error term, standard OLS regression provides biased estimates. 2SLS regression uses an "instrumental variable" (Z) that is correlated with X but not with the outcome (Y) to isolate the part of X that is exogenous and estimate its true causal effect.
                        </p>
                    </div>
                        ...
                        <div className="space-y-6">
                            <h3 className="font-semibold text-2xl flex items-center gap-2"><Settings className="text-primary"/> Setup Guide</h3>
                             <ol className="list-decimal list-inside space-y-4 text-muted-foreground">
                                ...
                                <li><strong>Endogenous Variable(s) (X):</strong> Predictors that are correlated with the unobserved error term.</li>
                                <li><strong>Exogenous Variable(s) (X):</strong> Standard predictors that are assumed to be uncorrelated with the error term.</li>
                         ...
                         <div className="space-y-6">
                            ...
                            <ul className="list-disc pl-5 space-y-4 text-muted-foreground">
                                <li><strong>2SLS vs. OLS Coefficients:</strong> Compare the coefficients from the 2SLS model to the naive OLS model. A significant difference suggests that OLS was biased and 2SLS provides a more reliable estimate of the causal effect.</li>
                                <li><strong>First-Stage F-statistic:</strong> (Coming soon) This tests if your instruments are strong enough. An F-statistic greater than 10 is a common rule of thumb.</li>
                            </ul>
                        </div>
                    </div>
                </CardContent>
                <CardFooter className="flex justify-between p-6 bg-muted/30 rounded-b-lg">
                     {ivExample && <Button variant="outline" onClick={() => onLoadExample(ivExample)}>Load Sample Regression Data</Button>}
...
export default function TwoStageLeastSquaresPage({ data, numericHeaders, onLoadExample }: { data: DataSet; numericHeaders: string[]; onLoadExample: (e: any) => void }) {
    ...
    const handleAnalysis = useCallback(async () => {
        ...
        try {
            const response = await fetch('/api/analysis/two-stage-least-squares', {
                method: 'POST',
    ...
    const handleLoadExampleData = () => {
        const ivExample = exampleDatasets.find(ex => ex.id === 'regression-suite');
        if (ivExample) {
            onLoadExample(ivExample);
            setView('main');
        }
    };
        ...
        <div className="space-y-4">
             <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                         <CardTitle className="font-headline">2SLS Regression Setup</CardTitle>
                ...
                <CardContent className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <Select value={xExogCols[0]} onValueChange={v => setXExogCols(v === 'none' ? [] : [v])}><SelectTrigger><SelectValue placeholder="None"/></SelectTrigger><SelectContent><SelectItem value="none">None</SelectItem>{numericHeaders.filter(h=>h!==yCol && !xEndogCols.includes(h)).map(h=><SelectItem key={h} value={h}>{h}</SelectItem>)}</SelectContent></Select>
                    </div>
                     <div>
            ...
            {analysisResult && (
                <div className="grid lg:grid-cols-2 gap-4">
                    {analysisResult.ols_results && renderResultsTable(analysisResult.ols_results)}
                    {analysisResult.tsls_results && renderResultsTable(analysisResult.tsls_results)}
                </div>
            )}
        </div>
    );
