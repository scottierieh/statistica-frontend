...
import { Loader2, Zap, HelpCircle, MoveRight, Settings, FileSearch, BarChart as BarChartIcon } from 'lucide-react';
...
interface CategoricalTableItem { name: string; count: number; percentage: number; }
...
const ChoiceAnalysisDisplay = ({ tableData, plotData, varName }: { tableData: CategoricalTableItem[], plotData?: string, varName: string }) => (
...
const NumberAnalysisDisplay = ({ plotData, tableData, varName }: { plotData?: string, tableData: NumericStats, varName: string }) => (
    <AnalysisDisplayShell varName={varName}>
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {plotData && <Image src={`data:image/png;base64,${plotData}`} alt={`${varName} plot`} width={500} height={400} className="rounded-md border" />}
...
const SummaryTable = ({ analysisData, selectedVars, numericHeaders, categoricalHeaders }: { analysisData: AnalysisResult, selectedVars: string[], numericHeaders: string[], categoricalHeaders: string[]}) => {
        ...
        return (
            <div className="space-y-8">
                {selectedVars.map(header => {
                    ...
                    if(result.type === 'numeric' && result.stats) { return <NumberAnalysisDisplay key={header} plotData={result.plot} tableData={result.stats} varName={header} /> }
                    if(result.type === 'categorical' && result.table) { return <ChoiceAnalysisDisplay key={header} plotData={result.plot} tableData={result.table} varName={header} /> }
                    return null;
                })}
            </div>
        );
