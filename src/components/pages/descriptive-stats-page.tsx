...
const getCategoricalStats = (data: (string | number)[]) => {
    ...
    return {
        ...
        summary: {
            count: validCount,
            missing: data.length - validCount,
            unique: stats.length,
            mode: stats.length > 0 ? stats[0].name : 'N/A'
        }
    };
};
...
const generateCategoricalInsights = (stats: ReturnType<typeof getCategoricalStats>['table']) => {
                                ...
                                {categoricalStatsOrder.map(statKey => (
                                    <TableRow key={statKey}>
                                        ...
                                        {categoricalSelected.map(header => {
                                            const value = analysisData[header]?.summary?.[statKey as keyof typeof analysisData[typeof header]['summary']];
...
const GroupedStatisticsTable = ({ data, selectedVars, groupVar }: { data: DataSet, selectedVars: string[], groupVar: string }) => {
    const groupedStats = useMemo(() => {
        ...
        for(const variable of selectedVars) {
            ...
            for(const group of groups) {
                ...
                const numericData = groupData.map(row => Number(row[variable])).filter(n => !isNaN(n));
                stats[variable][group] = getNumericStats(numericData);
            }
        }
        return stats;
    ...
    return (
        <Card>
            <CardHeader><CardTitle>Grouped Descriptive Statistics</CardTitle><CardDescription>Statistics for numeric variables, grouped by '{groupVar}'.</CardDescription></CardHeader>
            <CardContent>
                {selectedVars.map(variable => (
                    <div key={variable} className="mb-6">
                        <h3 className="font-semibold mb-2">{variable}</h3>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead>{groupVar}</TableHead>
                            ...
                            <TableBody>
                                {Object.entries(groupedStats[variable]).map(([group, stats]) => (
                                    <TableRow key={group}>
                                        <TableCell className="font-medium">{String(group)}</TableCell>
                                        <TableCell className="text-right font-mono">{(stats as any).count}</TableCell>
                                        <TableCell className="text-right font-mono">{(stats as any).mean.toFixed(2)}</TableCell>
                                        <TableCell className="text-right font-mono">{(stats as any).stdDev.toFixed(2)}</TableCell>
                                        <TableCell className="text-right font-mono">{(stats as any).median.toFixed(2)}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
