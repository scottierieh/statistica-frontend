        ...
        <div className="space-y-4">
            <Tabs defaultValue="importance" className="w-full">
                ...
                <TabsContent value="partworths" className="mt-4">
                    <Card><CardHeader><CardTitle className='flex items-center gap-2'><BarIcon/>Part-Worth Utilities</CardTitle><CardDescription>Utility values for each attribute level (zero-centered)</CardDescription></CardHeader>
                        <CardContent><div className="grid md:grid-cols-2 gap-6">{attributeCols.map(attr => { const attrData = partWorthsData.filter(p => p.attribute === attr); return (<div key={attr} className="space-y-2"><h3 className="font-semibold text-lg">{attr}</h3><ChartContainer config={partWorthChartConfig} className="w-full h-[250px]"><ResponsiveContainer><BarChart data={attrData} layout="vertical" margin={{ left: 100, right: 20, top: 20, bottom: 20 }}><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis dataKey="level" type="category" width={90} tick={{ fontSize: 12 }}/><Tooltip content={<ChartTooltipContent />} /><Bar dataKey="value" name="Part-Worth" fill="hsl(var(--primary))" barSize={30}/></BarChart></ResponsiveContainer></ChartContainer></div>);})}</div></CardContent>
                    </Card>
                </TabsContent>
                ...
                {results.segmentation && (
                    <TabsContent value="segmentation" className="mt-4">
                        <Card><CardHeader><CardTitle className='flex items-center gap-2'><Users className="h-5 w-5"/>Segmentation Analysis</CardTitle><CardDescription>Analysis results broken down by segment</CardDescription></CardHeader><CardContent><Alert><AlertTriangle className="h-4 w-4" /><AlertDescription>Segmentation analysis visualization coming soon!</AlertDescription></Alert></CardContent></Card>
                    </TabsContent>
                )}
            </Tabs>
