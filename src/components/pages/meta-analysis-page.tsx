...
import { Sigma, Loader2, BarChart, LineChart, PieChart, Users } from 'lucide-react';
...
const ForestPlot = ({ studies, fixed, random }: { studies: AnalyzedStudy[], fixed: ModelResult, random: ModelResult }) => {
    ...
    const totalWeight = chartData.reduce((sum, d) => sum + d.weight, 0);
...
export default function MetaAnalysisPage({ onLoadExample }: { onLoadExample: (example: any) => void }) {
    ...
    // Analysis settings
    const [analysisTitle, setAnalysisTitle] = useState('My Meta-Analysis');
    const [effectSizeType, setEffectSizeType] = useState('cohen_d');
    const [analysisModel, setAnalysisModel] = useState('random');
    ...
    const handleLoadSample = () => {
        setNumStudies(6);
        setAnalysisTitle("Online Learning Meta-Analysis");
        setEffectSizeType("cohen_d");
        const sampleData = [
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid md:grid-cols-2 gap-4">
                           <div><Label>Analysis Title</Label><Input value={analysisTitle} onChange={e => setAnalysisTitle(e.target.value)} /></div>
                           <div><Label>Effect Size Type</Label><Select value={effectSizeType} onValueChange={setEffectSizeType}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="cohen_d">Cohen's d</SelectItem><SelectItem value="hedges_g">Hedges' g</SelectItem><SelectItem value="odds_ratio">Odds Ratio</SelectItem><SelectItem value="correlation">Correlation</SelectItem></SelectContent></Select></div>
                    ...
                    <CardFooter className="flex justify-between">
                        ...
                        <Button onClick={() => setCurrentStep(2)}>Next: Data Input</Button>
                    </CardFooter>
                </Card>
            )}
            ...
            {currentStep === 3 && analysisResult && (
                <Card>
                    ...
                    <CardFooter className="flex justify-between">
                        ...
                        <Button onClick={() => setCurrentStep(4)}>Next: Visualization</Button>
                    </CardFooter>
                </Card>
            )}
            ...
            {currentStep === 4 && analysisResult && (
                <Card>
                    ...
                    <CardFooter className="flex justify-start">
                        <Button variant="outline" onClick={() => setCurrentStep(3)}>Back to Results</Button>
                    </CardFooter>
                </Card>
            )}
        </div>
