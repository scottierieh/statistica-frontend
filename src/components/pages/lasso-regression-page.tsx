...
export default function LassoRegressionPage({ data, numericHeaders, onLoadExample }: LassoRegressionPageProps) {
    ...
    const handleAnalysis = useCallback(async () => {
        ...
        try {
            ...
            toast({ title: 'Analysis Complete', description: 'Lasso regression model has been trained and evaluated.' });
            ...
            <Card>
                ...
                <CardFooter className="flex justify-end">
                    <Button onClick={handleAnalysis} disabled={isLoading || !target || features.length === 0}>
                        {isLoading ? <><Loader2 className="mr-2 animate-spin"/> Running...</> : <><Sigma className="mr-2"/>Run Lasso Regression</>}
                    </Button>
                </CardFooter>
            </Card>
