     ...
     if (isPreview) {
        return (
            <div>
                ...
                <Input type="number" placeholder={question.text || "Enter a number..."} value={answer || ''} onChange={e => onAnswerChange?.(e.target.value)} />
            </div>
        );
    }
        ...
        <Card className="bg-white">
            <CardContent className="p-6 space-y-4">
                ...
                <div>
                    ...
                    <Input id={`placeholder-${question.id}`} placeholder="e.g., Enter your age" value={question.text || ''} onChange={(e) => onUpdate?.({...question, text: e.target.value})}/>
                </div>
            </CardContent>
        </Card>
    );
