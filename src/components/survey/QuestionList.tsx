...
import { GripVertical, PlusCircle, Save, ImageIcon, Image as ImageIconFeather } from "lucide-react";
...
interface QuestionListProps {
  ...
  styles: any;
  saveSurvey?: (status?: 'draft' | 'active' | 'closed' | 'scheduled') => void;
  isSaving?: boolean;
}
...
export default function QuestionList({ survey, setSurvey, onImageUpload, onDuplicate, styles, saveSurvey, isSaving }: QuestionListProps) {
    ...
    const { questions, title, description } = survey;
        ...
        <div className="space-y-6">
            <Card>
                ...
                <CardContent className="space-y-4">
                    <div><Label>Title *</Label><Input value={title} onChange={(e) => handleSurveyChange(draft => { draft.title = e.target.value })} /></div>
                    <div><Label>Description</Label><Textarea value={description} onChange={(e) => handleSurveyChange(draft => { draft.description = e.target.value })} /></div>
                </CardContent>
            </Card>
