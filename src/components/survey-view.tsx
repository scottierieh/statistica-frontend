                ...
                <div className="relative py-5">
                    <div className="flex justify-between items-center px-2">
                        {[-9, -7, -5, -3, 1, 3, 5, 7, 9].map(value => {
                             ...
                             const actualValue = value < 0 ? 1 / Math.abs(value) : value;
                             
                             return (
                                <div key={value} onClick={() => onAnswerChange(produce(answer || {}, (draft: any) => { draft[row] = String(actualValue); }))}
                                    className={cn( "w-10 h-10 rounded-full bg-white border-2 flex items-center justify-center cursor-pointer transition-all z-10", selectedValue === String(actualValue) ? "bg-primary text-white border-primary-dark scale-110 shadow-lg" : "border-slate-300 hover:border-primary hover:scale-110" )}>
                                    {displayValue}
                                </div>
                            )
                        })}
